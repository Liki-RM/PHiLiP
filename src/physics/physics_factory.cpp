                 ...
                 std::shared_ptr< ModelBase<dim,nstate,real> >                model_input)
{
    ...
    if (pde_type == PDE_enum::advection || pde_type == PDE_enum::advection_vector) {
        ...
        // constexpr std::array<int, 3> possible_nspecies = {2, 5, 11}; // warning: must correspond to instantiations of the class
        constexpr int nspecies = nstate-dim-2+1;
        if constexpr (nspecies==2 ||
                      (nspecies==5 || 
                       nspecies==11)) {
            return std::make_shared < RealGas<dim,nstate,real,nspecies> > (
                parameters_input,
                manufactured_solution_function);
        }
